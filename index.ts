...
export function extendWithFsExtraApi<T extends IFs>(fs: T | FsSynchronousApi | IFakeUnionfsLike, opts?: IOptionMemFsExtra): IMemFsExtra<T>
{
	...
	fse.StatWatcher ??= _fs.StatWatcher;
	fse.FSWatcher ??= _fs.FSWatcher;
	fse._toUnixTimestamp ??= _fs._toUnixTimestamp;
 ...
 */
export function _getUnsupportMethods(): readonly ['FileReadStream', 'FileWriteStream', 'Utf8Stream', 'Dir', 'gracefulify']
{
	return [
		'FileReadStream',
		'FileWriteStream',
